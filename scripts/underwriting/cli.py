"""
cli.py — CLI entry point: deal.yaml → underwriting.xlsx + console summary.

Usage:
    python -m scripts.underwriting examples/marina-apartments.yaml
    python -m scripts.underwriting deal.yaml -o outputs/marina.xlsx
"""
...
def _print_summary(pf, wf):
    ...
    print(f"  {'Units':<32} {deal.property.unit_count}")
    ...
    print(f"  {'GP Net IRR (co-inv + promote)':<32} {wf.gp.irr*100:>16.2f}%")
    print(f"  {'GP Net MOIC':<32} {wf.gp.moic:>16.2f}x")
    print(bar)


def main():
    ap = argparse.ArgumentParser(description="Underwrite a deal: produces institutional XLSX + console summary.")
